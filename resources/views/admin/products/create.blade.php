            ...
            <div class="admin-card mb-4">
                ...
                <div class="admin-card-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control form-control-sm @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name') }}" placeholder="Ví dụ: Dây Chuyền Bạc Thái Dương Lam..." required autofocus>
                        @error('name')
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control form-control-sm font-monospace text-uppercase @error('sku') is-invalid @enderror" id="sku" name="sku" value="{{ old('sku') }}" placeholder="VD: LNS-DC001" required>
                            @error('sku')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            <label for="slug" class="form-label fw-semibold small">Đường dẫn (Slug)</label>
                            <input type="text" class="form-control form-control-sm font-monospace @error('slug') is-invalid @enderror" id="slug" name="slug" value="{{ old('slug') }}" placeholder="Tự động tạo nếu để trống">
                            @error('slug')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select form-select-sm @error('product_type') is-invalid @enderror" id="product_type" name="product_type" required>
                                <option value="single" {{ old('product_type', 'single') === 'single' ? 'selected' : '' }}>Sản phẩm đơn (Single)</option>
                                <option value="collection" {{ old('product_type') === 'collection' ? 'selected' : '' }}>Bộ sưu tập (Collection)</option>
                                <option value="gift" {{ old('product_type') === 'gift' ? 'selected' : '' }}>Quà tặng (Gift)</option>
                            </select>
                            @error('product_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                ...
                <div class="admin-card-body">
                    <div class="row g-3 mb-2">
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control form-control-sm font-monospace @error('regular_price') is-invalid @enderror" id="regular_price" name="regular_price" value="{{ old('regular_price') }}" min="0" step="1000" placeholder="0" required>
                            @error('regular_price')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control form-control-sm font-monospace @error('sale_price') is-invalid @enderror" id="sale_price" name="sale_price" value="{{ old('sale_price') }}" min="0" step="1000" placeholder="Để trống nếu không giảm">
                            @error('sale_price')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row g-3" id="singleStockContainer">
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control form-control-sm font-monospace @error('stock_quantity') is-invalid @enderror" id="stock_quantity" name="stock_quantity" value="{{ old('stock_quantity', 0) }}" min="0">
                            <div class="form-text small">Tồn kho = 0 sẽ tự động đổi sang trạng thái Hết hàng (Out of Stock).</div>
                    ...
                    <div class="alert alert-secondary border-0 mt-3 mb-0 small" id="bundleStockNotice" style="display: none;">
                        <i class="bi bi-info-circle-fill me-1 text-primary"></i>
                        <strong>Sản phẩm dạng bộ:</strong> Tồn kho được tính toán tự động dựa trên số lượng các sản phẩm đơn lẻ thành phần có trong kho.
                    </div>
                </div>
            </div>
            ...
            <div class="admin-card mb-4">
                ...
                <div class="admin-card-body">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control form-control-sm" id="short_description" name="short_description" rows="2" placeholder="Tóm tắt ngắn gọn...">{{ old('short_description') }}</textarea>
                    </div>

                    <div class="mb-3">
                        <label for="description" class="form-label fw-semibold small">Nội dung chi tiết</label>
                        <textarea class="form-control form-control-sm" id="description" name="description" rows="4" placeholder="Câu chuyện sản phẩm, hướng dẫn bảo quản...">{{ old('description') }}</textarea>
                    </div>

                    <div class="row g-3">
                        <div class="col-sm-6">
                            <label for="material" class="form-label fw-semibold small">Chất liệu</label>
                            <input type="text" class="form-control form-control-sm" id="material" name="material" value="{{ old('material', 'Bạc Ý 925 cao cấp') }}">
                        </div>

                        <div class="col-sm-6">
                            <label for="stone" class="form-label fw-semibold small">Đá đính kèm</label>
                            <input type="text" class="form-control form-control-sm" id="stone" name="stone" value="{{ old('stone') }}" placeholder="VD: Đá CZ tinh khiết...">
                        </div>

                        <div class="col-sm-6">
                            <label for="weight" class="form-label fw-semibold small">Trọng lượng</label>
                            <input type="text" class="form-control form-control-sm" id="weight" name="weight" value="{{ old('weight') }}" placeholder="VD: 3.2g">
                        </div>

                        <div class="col-sm-6">
                            <label for="size_info" class="form-label fw-semibold small">Kích thước / Size</label>
                            <input type="text" class="form-control form-control-sm" id="size_info" name="size_info" value="{{ old('size_info') }}" placeholder="VD: Freesize điều chỉnh...">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="admin-card mb-4">
                ...
                <div class="admin-card-body">
                    <div class="form-check form-switch mb-3">
                        ...
                        <div class="form-text small">Nếu tắt, sản phẩm sẽ ẩn khỏi storefront.</div>
                    </div>
    ...
    <div class="admin-sticky-save-bar">
        ...
        <div class="d-flex align-items-center gap-2">
            <button type="submit" class="btn btn-sm btn-primary px-4 fw-semibold" id="btnSubmitForm">
                <i class="bi bi-check2 me-1"></i> Lưu sản phẩm
            </button>
        </div>
    </div>
</form>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let bundleIndex = {{ count(old('bundle_items', [])) + 10 }};
